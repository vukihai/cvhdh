...
namespace APITests.Factories
{
	public class EntityFactory : IXunitSerializable
	{
		...
		public BaseEntity Construct(bool isValid = true)
		{
			return _type switch
			{
				"AchievementsEntity" => AchievementsEntity.GetEntity(isValid, _fixedStrValues),
				"AddressEntity" => AddressEntity.GetEntity(isValid, _fixedStrValues),
				"StudentsEntity" => StudentsEntity.GetEntity(isValid, _fixedStrValues),
				"AssessmentNotesEntity" => AssessmentNotesEntity.GetEntity(isValid, _fixedStrValues),
				"AssessmentsEntity" => AssessmentsEntity.GetEntity(isValid, _fixedStrValues),
				"CommentsEntity" => CommentsEntity.GetEntity(isValid, _fixedStrValues),
				"StaffEntity" => StaffEntity.GetEntity(isValid, _fixedStrValues),
				"AchievementsSubmissionEntity" => AchievementsSubmissionEntity.GetEntity(isValid, _fixedStrValues),
				"AddressSubmissionEntity" => AddressSubmissionEntity.GetEntity(isValid, _fixedStrValues),
				"StudentsSubmissionEntity" => StudentsSubmissionEntity.GetEntity(isValid, _fixedStrValues),
		...
		public string GetEnumValue(BaseEntity entity, string enumColumnName)
		{
			switch (_type)
			{
				case "AchievementsEntity":
					switch (enumColumnName)
					{
						case "Achievement Type":
							return ((AchievementsEntity)entity).AchievementType.ToString();
						default:
							return null;
					}
				case "StaffEntity":
					...
					{
						case "Role":
							return ((StaffEntity)entity).Role.ToString();
						default:
							return null;
					}
				default:
