...
namespace APITests.EntityObjects.Models
{
	public class CommentsEntity : BaseEntity
	{
		...
		{
			EntityName = "CommentsEntity";
			InitialiseReferences();
		}

		public CommentsEntity(ConfigureOptions option)
		{
			Configure(option);
			InitialiseReferences();
		}
		...
		{
			return GetInvalidEntities<CommentsEntity>()
		...
		public override Dictionary<string, string> ToDictionary()
		{
			...
			{
				{"id" , Id.ToString()},
				{"commentDate" ,((DateTime)CommentDate).ToIsoString()},
		...
		public override RestSharp.JsonObject ToJson()
		{
			...
			if(CommentDate != null) 
			{
				entityVar["commentDate"] = CommentDate?.ToString("s");
			}
			if(CommentDescription != null) 
			{
				entityVar["commentDescription"] = CommentDescription.ToString();
			}
			if (AssessmentsId  != default)
			{
				entityVar["assessmentsId"] = AssessmentsId.ToString();
		...
		public static CommentsEntity GetEntity(bool isValid, string fixedValue = null)
		{
			if (isValid && !string.IsNullOrEmpty(fixedValue))
			{
				return GetValidEntity(fixedValue);
			}
			return isValid ? GetValidEntity() : GetInvalidEntity<CommentsEntity>().entity;
		}
		...
		/// Gets an entity with attributes that conform to any attribute validators.
		/// </summary>
		public static CommentsEntity GetValidEntity(string fixedStrValue = null)
		{
			var commentsEntity = new CommentsEntity
			{
			};
			commentsEntity.PopulateAttributes();
