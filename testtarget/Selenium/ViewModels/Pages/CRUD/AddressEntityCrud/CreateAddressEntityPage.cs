...
{
	public class CreateAddressEntityPage : AddressEntityAttributeCollection
	{
		// % protected region % [Override class properties here] off begin
		public string Url => ContextConfiguration.BaseUrl + "/admin/addressEntity/create";
		public CrudCreateButtons ActionButtons => new(ContextConfiguration);
		// % protected region % [Override class properties here] end

		// % protected region % [Override constructor here] off begin
		public CreateAddressEntityPage(ContextConfiguration contextConfiguration) : base(contextConfiguration)
		{
		}
		...
		public void SetValues(AddressEntity addressEntity)
		{
			Name.Value = addressEntity.Name;
			Unit.Value = addressEntity.Unit;
			AddressLine1.Value = addressEntity.AddressLine1;
			AddressLine2.Value = addressEntity.AddressLine2;
			Suburb.Value = addressEntity.Suburb;
			Postcode.Value = addressEntity.Postcode.ToString();
			City.Value = addressEntity.City;
			Country.Value = addressEntity.Country;
			StaffsIds.Value = addressEntity.StaffsIds?.Select(x => x.ToString());
			StudentsIds.Value = addressEntity.StudentsIds?.Select(x => x.ToString());
		}
