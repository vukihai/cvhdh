...
try {
	...
	do {
		Start-Sleep 1;
		$serverStatus = docker inspect --format='{{.State.Health.Status}}' firstapp2257_server_1
		...
		$clientStatus = docker inspect --format='{{.State.Health.Status}}' firstapp2257_client_1;
	...
	} Until((docker-compose --project-name "firstapp2257" ps | Select-String 'Up \(healthy\)' | Measure-Object -Line).Lines -gt 1)
