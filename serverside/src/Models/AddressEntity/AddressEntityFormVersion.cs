...
namespace Firstapp2257.Models {
	...
	// % protected region % [Modify class declaration here] end
	{
		...
		public AddressEntityFormVersion()
		{
		...
		[EntityForeignKey("Form", "FormVersions", false, typeof(AddressEntity))]
		public AddressEntity Form { get; set; }

		/// <summary>
		/// The current form that this version is published against if it is published
		/// </summary>
		/// <see cref="Firstapp2257.Models.AddressEntity"/>
		[EntityForeignKey("PublishedForm", "PublishedVersion", false, typeof(AddressEntity))]
		public AddressEntity? PublishedForm { get; set; }

		/// <summary>
		/// Reference to the submissions for this form version
		/// </summary>
		/// <see cref="Firstapp2257.Models.AddressSubmissionEntity"/>
		[EntityForeignKey("FormSubmissions", "FormVersion", false, typeof(AddressSubmissionEntity))]
		public ICollection<AddressSubmissionEntity> FormSubmissions { get; set; }
			...
			CancellationToken cancellationToken = default)
		{
			...
			if (PublishVersion)
			{
				var formModel = dbContext.AddressEntity.FirstOrDefault(m => m.Id == FormId);
				if (formModel != null)
				{
		...
		public async Task<int> CleanReference<T>(
			...
			{
			var modelList = models.Cast<AddressEntityFormVersion>().ToList();
